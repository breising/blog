		...
		if u:
			...
			if dbHashedPassword:
				...
				if validate_bcrypt(submittedPassword, dbHashedPassword):
					# make a new user-key hash to create a login cookie for this user for the next time they return
					str_user_id = str(user_id)
					userKeyHash_cookie_val = str(make_secure_val(str_user_id))
					# set the cookie
					self.response.headers.add_header('Set-Cookie', 'user_id=%s' % userKeyHash_cookie_val)
